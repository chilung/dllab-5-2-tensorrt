...
from .. import infer
from .. import utils
from ..parsers import uffparser, caffeparser
...
class Engine(object):
    ...
    def __init__(self, **kwargs):
        ...
        Args:
            ...
            - **logger** ``<class> tensorrt.infer.Logger`` *optional*: Engine Logger *Default: ``tensorrt.infer.Logger``*
...
pip install tensorrt[examples]""".format(err))
        ...
        self.runtime = infer.create_infer_runtime(self.logger)
...
https://docs.nvidia.com/deeplearning/sdk/tensorrt-api/#python and click on the 'TensoRT Python API' link""".format(err))
            ...
            self.engine = utils.load_engine(self.logger, kwargs.get("PLAN", None), kwargs.get("plugins", None))
        ...
        num_bindings = self.engine.get_nb_bindings()
        ...
        self.input_dim = [self.engine.get_binding_dimensions(i) for i in input_index]
        self.output_dim = [self.engine.get_binding_dimensions(o) for o in output_index]

        insize = [self.max_batch_size * vol(i) * self.alloc_data_type.itemsize for i in self.input_dim]
        outsize = [self.max_batch_size * vol(o) * self.alloc_data_type.itemsize for o in self.output_dim]
    ...
    def _create_engine(self, modelstream, **kwargs):
        ...
        if self.src_framework == "uff":
            ...
            if modelstream:
                self.engine = utils.uff_to_trt_engine(self.logger,
                                                          modelstream,
                                                          parser,
                                                          self.max_batch_size,
                ...
                self.engine = utils.uff_file_to_trt_engine(self.logger,
                                                               kwargs["path"],
                                                               parser,
                                                               self.max_batch_size,
                                                               self.max_workspace_size,
            ...
            self.engine = utils.caffe_to_trt_engine(self.logger,
                                                        kwargs["deployfile"],
                                                        kwargs["modelfile"],
                                                        self.max_batch_size,
                                                        self.max_workspace_size,
    ...
    def save(self, path):
        ...
        *Note:* This saves the only the internal TensorRT engine in the class
        ...
        '''
        utils.write_engine_to_file(path, self.engine.serialize())
...
for input layes batch sizes not uniform, expected {}, saw {}'''.format(len(input_data[0].shape[0]),i.shape[0]))
    ...
    def infer(self, *input_data):
        ...
        for o in self.output_dim:
            ...
            for b in input_data[0]:
                layer_out.append(np.empty((b.shape[0], o[0], o[1], o[2]), dtype=self.alloc_data_type.input_type()))
    ...
    def log_info(self, msg):
        ...
        '''
        self.logger.log(infer.LogSeverity.INFO, msg)
    ...
    def log_warn(self, msg):
        ...
        '''
        self.logger.log(infer.LogSeverity.WARNING, msg)
    ...
    def log_error(self, msg):
        ...
        Raises:
            - ValueError
        '''
        self.logger.log(infer.LogSeverity.ERROR, msg)
