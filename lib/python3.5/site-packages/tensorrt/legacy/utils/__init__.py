...
from .. import infer
from ..parsers import caffeparser, uffparser

from .._deprecated_helpers import warn_deprecated
...
def trt_network_to_trt_engine(logger, network, max_batch_size, max_workspace_size, datatype=infer.DataType.FLOAT, plugin_factory=None, calibrator=None):
    ...
    Args:
        - **logger** ``infer.Logger``: Logging system for the application
        - **trt_graph** ``str``: network
        - **max_batch_size** ``int``: Maximum batch size
        - **max_workspace_size** ``int``: Maximum workspace size
        - **datatype** ``infer.DataType``: Operating data type of the engine, can be FP32, FP16 if supported on the platform, or INT8 with calibrator. Default: ``infer.DataType.FLOAT``
        - **plugins_factory** ``infer.PluginFactory`` *(optional)*: Custom layer factory. Default: `` None``
        - **calibrator** ``infer.Int8Calibrator`` *(optional)*: Currently unsupported. Default: ``None``

    Returns:
        - ``infer.CudaEngine``: TensorRT Engine to be used or excuted
    '''
    builder = infer.create_infer_builder(logger)


    if datatype == infer.DataType.HALF and not builder.platform_has_fast_fp16():
        logger.log(infer.LogSeverity.ERROR, "Specified FP16 but not supported on platform")
        raise AttributeError("Specified FP16 but not supported on platform")

    if datatype == infer.DataType.INT8 and calibrator == None:
        logger.log(infer.LogSeverity.ERROR, "Specified INT8 but no calibrator provided")
        raise AttributeError("Specified INT8 but no calibrator provided")


    if datatype == infer.DataType.INT32:
        logger.log(infer.LogSeverity.ERROR, "INT32 is not supported at this time ")
    ...
    if datatype == infer.DataType.HALF:
        builder.set_fp16_mode(True)

    if datatype == infer.DataType.INT8 and calibrator:
    ...
    try:
        ...
        logger.log(infer.LogSeverity.ERROR, "Failed to create engine")
        _, _, tb = sys.exc_info()
...
def uff_file_to_trt_engine(logger, uff_file, parser, max_batch_size, max_workspace_size, datatype=infer.DataType.FLOAT, plugin_factory=None, calibrator=None):
    ...
    if datatype == infer.DataType.INT32:
        logger.log(infer.LogSeverity.ERROR, "INT32 is not supported in uff parser at this time ")
        raise AttributeError("INT32 is not supported in uff parser at this time")

    model_datatype = infer.DataType.FLOAT
    if datatype == infer.DataType.HALF:
        model_datatype = infer.DataType.HALF

    try:
        assert(parser.parse_from_file(uff_file, network, model_datatype))
    except AssertionError:
        logger.log(infer.LogSeverity.ERROR, "Failed to parse UFF File '{}'".format(uff_file))
        _, _, tb = sys.exc_info()
        traceback.print_tb(tb) # Fixed format
        tb_info = traceback.extract_tb(tb)
        filename, line, func, text = tb_info[-1]
    ...
    if datatype == infer.DataType.INT8:
...
def uff_to_trt_engine(logger, stream, parser, max_batch_size, max_workspace_size, datatype=infer.DataType.FLOAT, plugin_factory=None, calibrator=None):
    ...
    try:
        ...
        logger.log(infer.LogSeverity.ERROR, "Failed to parse UFF model stream")
        _, _, tb = sys.exc_info()
        traceback.print_tb(tb) # Fixed format
        tb_info = traceback.extract_tb(tb)
        filename, line, func, text = tb_info[-1]
...
def caffe_to_trt_engine(logger, deploy_file, model_file, max_batch_size, max_workspace_size, output_layers, datatype=infer.DataType.FLOAT, plugin_factory=None, calibrator=None):
    ...
    Args:
        - **logger** ``infer.Logger``: A logger is needed to monitor the progress of building the engine
    ...
    Returns
        - ``infer.CudaEngine``: An engine that can be used to execute inference
    ...
    logger.log(infer.LogSeverity.INFO, "Parsing caffe model {}, {}".format(deploy_file, model_file))

    try:
        assert(blob_name_to_tensor)
    except AssertionError:
        logger.log(infer.LogSeverity.ERROR, "Failed to parse caffe model")
        _, _, tb = sys.exc_info()
        traceback.print_tb(tb) # Fixed format
        tb_info = traceback.extract_tb(tb)
        filename, line, func, text = tb_info[-1]
    ...
    for i in range(network.get_nb_inputs()):
        ...
        logger.log(infer.LogSeverity.INFO, "Input \"{}\":{}x{}x{}".format(network.get_input(i).get_name(), dims.C(), dims.H(), dims.W()))
    ...
    for l in output_layers:
        logger.log(infer.LogSeverity.INFO, "Marking " + l + " as output layer")
        t = blob_name_to_tensor.find(l)
        try:
            assert(t)
        except AssertionError:
            logger.log(infer.LogSeverity.ERROR, "Failed to find output layer {}".format(l))
            _, _, tb = sys.exc_info()
            traceback.print_tb(tb) # Fixed format
            tb_info = traceback.extract_tb(tb)
            filename, line, func, text = tb_info[-1]
    ...
    for i in range(network.get_nb_outputs()):
        ...
        logger.log(infer.LogSeverity.INFO, "Output \"{}\":{}x{}x{}".format(network.get_output(i).get_name(), dims.C(), dims.H(), dims.W()))
    ...
    if datatype == infer.DataType.HALF:
        logger.log(infer.LogSeverity.INFO, "Enabling FP16 mode")
        builder.set_fp16_mode(True)
    ...
    logger.log(infer.LogSeverity.INFO, "Building engine")
...
def load_engine(logger, filepath, plugins = None):
    ...
    Args:
        ...
        - **plugins** ``infer.PluginFactory`` *(optional)*: Custom layer factory
    ...
    with infer.Runtime(logger) as runtime, open(filepath, "rb") as inpf:
        return runtime.deserialize_cuda_engine(inpf.read(), 0, plugins)
...
def write_engine_to_file(filepath, engine):
    ...
    Args:
        - **filepath** ``str``: Path to engine file
        - **engine** ``infer.CudaEngine``: An engine that can be used to execute inference
    ...
    with open(filepath, "wb") as outf:
        try:
            ...
            # Because apparently engine is sometimes an IHostMemory object instead of an engine...
            outf.write(engine)
    return True
...
#     and associated weights encoded in infer.Weights object
#
#     Args:
#         - **filepath** ``str``: path to the weights file
#
#     Returns:
#         - ``dict {str, infer.Weights}``: Dictionary of layer names and associated weights
#
#     """
#     filepath = _file_exists(filepath)
