...
input_data = np.random.random(size=(2, 3, 224, 224)).astype(np.float32)
...
# f_handler=open('./rt_batch_size.log', 'w')
...
# for test_batch in []:
    ...
    model = onnx.load("edsr.model.onnx")
    # model = onnx.load("resnet18.onnx")
...
# f_handler=open('./rt_work_space.log', 'w')
# sys.stdout=f_handler

test_batch = 32
for work_space in []:
# for work_space in [0, 10, 15, 20, 27, 30, 31]:
# 1, 1K, 32K,  1M, 128M, 1G, 2G
    x = 'test'
    dataloaders[x] = torch.utils.data.DataLoader(image_datasets[x], batch_size=test_batch,
                                                 shuffle=True, num_workers=0)
    
    model = onnx.load("resnet18.onnx")
    engine = backend.prepare(model, device='CUDA:0', max_batch_size=test_batch, set_max_workspace_size=2**work_space)

    tensorrt_model = engine
    print('work space: 2^{}'.format(work_space))
    t0 = time.time()
    test_tensorrt(tensorrt_model, 'test')
    print('{} seconds'.format(time.time() - t0))
f_handler.close()
